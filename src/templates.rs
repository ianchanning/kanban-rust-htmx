...
fn render_sprite_pill(sprite: &Sprite) -> String {
...
fn render_add_note_form(wip_group_id: i64) -> String {
    ...
    let hx_target_notes_for_wip_group = format!("#notes-for-wip-group-{}", wip_group_id);
    ...
    format!(
        ...
        "#,
        hx_target_notes_for_wip_group, wip_group_id, input_class, button_class
    )
}
...
pub fn render_wip_group_card(wip_group: &WipGroup) -> Html<String> {
    Html(format!(
        ...
        <div id="wip-group-{0}" class="flex-1 bg-gray-800 p-4 rounded-lg shadow-lg border border-gray-700">
            ...
            <div id="sprites-for-wip-group-{0}" class="mb-4 flex flex-wrap gap-2"></div>
            <div id="notes-for-wip-group-{0}"></div>
            {2}
        </div>
        "#,
        wip_group.id, 
        html_escape(&wip_group.name),
        render_add_note_form(wip_group.id)
    ))
}
        ...
        Ok(wip_groups) => {
            ...
            for wip_group in wip_groups {
                html.push_str(&format!(
                    r#"<div id="wip-group-{0}" class="flex-1 bg-gray-800 p-4 rounded-lg shadow-lg border border-gray-700">
                        <h2 class="text-2xl font-semibold mb-4 text-white">{1}</h2>
                ...
                match Sprite::find_by_wip_group_id(&pool, wip_group.id).await {
                    Ok(sprites) => {
                        for sprite in sprites {
                            html.push_str(&render_sprite_pill(&sprite));
                        }
                    },
                ...
                // Add Note Form
                html.push_str(&render_add_note_form(wip_group.id));
